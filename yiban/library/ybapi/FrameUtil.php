<?php
	...
	class Library_Ybapi_FrameUtil
	{
		...
		{	
			if (!isset($_POST['verify_request']) || empty($_POST['verify_request']))
			{
				throw new YBException(Library_Lang::E_EXE_PERFORM);
			}
			$encText = addslashes($_POST['verify_request']);
			$strText = pack("H*", $encText);
			$decText = mcrypt_decrypt(MCRYPT_RIJNDAEL_256, $this->seckey, $strText, MCRYPT_MODE_CBC, $this->appid);
			if (empty($decText))
			{
				throw new YBException(Library_Lang::E_DEC_STRING);
			}
			$decInfo = json_decode(trim($decText), true);
			if (!is_array($decInfo) || !isset($decInfo['visit_oauth']))
			{
				throw new YBException(Library_Lang::E_DEC_RESULT);
			}
...
<title>{Library_Lang::WEB_APP_TITLE}</title>
