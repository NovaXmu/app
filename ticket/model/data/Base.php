...
class Data_Base
{
	...
	protected static function getUserInfo()
	{
		...
		if (isset(self::$resource['openid'])) {
			$result = $db->select('User', '*', array('wechatOpenid' => self::$resource['openid']));
		}
		else {
			$result = $db->select('User', '*', array('id' => self::$resource['ID']));
		}
	...
	public function isLinkYb(){
		return $this->_getInfo('yiban_isLinked');
	}
